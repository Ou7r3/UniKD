...
class BaseSolver(object):
    ...
    def _setup(self):
        ...
        if hasattr(self.model, 'load_teacher_checkpoint'):
            teacher_ckpt = getattr(self.model, 'teacher_ckpt', None)
            if teacher_ckpt:
                ckpt_path = Path(teacher_ckpt)
                if ckpt_path.is_file():
                    print(f'Loading teacher checkpoint from {ckpt_path}')
                    self.model.load_teacher_checkpoint(str(ckpt_path))
                else:
                    print(f"[Warning] Teacher checkpoint not found: {ckpt_path}")

        if hasattr(self.model, 'load_student_checkpoint'):
            student_ckpt = getattr(self.model, 'student_ckpt', None)
            if student_ckpt:
                ckpt_path = Path(student_ckpt)
                if ckpt_path.is_file():
                    print(f'Warm-starting student checkpoint from {ckpt_path}')
                    self.model.load_student_checkpoint(str(ckpt_path), strict=False)   #注意
                else:
                    print(f"[Warning] Student checkpoint not found: {ckpt_path}")
    ...
    def _sync_ema_with_model(self) -> None:
        ema = getattr(self, 'ema', None)
        if ema is None or not hasattr(ema, 'module'):
            return
        model = dist_utils.de_parallel(getattr(self, 'model', None))
        if model is None:
            return
        with torch.no_grad():
