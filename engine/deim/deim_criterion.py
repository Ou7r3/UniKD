...
from ..distill import FGDFeatureLoss
...
class DEIMCriterion(nn.Module):
    ...
    def __init__(self, \
        ...
        self.distill_ramp_epochs = distill_ramp_epochs
        self.distill_schedule = {}

        for cfg in self.distill_cfg:
            name = cfg.get('name')
            if name is None:
                raise ValueError('distill_cfg entries must provide a `name`.')
            loss_cfg = dict(cfg.get('loss', {}))
            loss_type = loss_cfg.pop('type', 'FGDFeatureLoss')
            if loss_type != 'FGDFeatureLoss':
                raise ValueError(f'Unsupported distillation loss: {loss_type}')
            self.distill_losses[name] = FGDFeatureLoss(**loss_cfg)
            self.distill_meta[name] = {'weight': cfg.get('weight', 1.0)}
            self.distill_schedule[name] = {
                'start_epoch': cfg.get('start_epoch', self.distill_start_epoch),
                'ramp_epochs': cfg.get('ramp_epochs', self.distill_ramp_epochs),
                'start_weight': cfg.get('start_weight', 0.0),
            }
    ...
    def forward(self, outputs, targets, epoch=0, **kwargs):
        ...
        distill_losses = self._compute_distill_losses(outputs, targets, epoch=epoch)
        losses.update(distill_losses)
        losses = {k:torch.nan_to_num(v, nan=0.0) for k, v in losses.items()}
        return losses

    def _compute_distill_losses(self, outputs, targets, epoch: int = 0):
        if not self.distill_losses or 'distill_pairs' not in outputs:
            return {}

        device = outputs['pred_boxes'].device
        device_type = device.type
        use_amp = (device_type == 'cuda')
        gt_boxes, img_metas = self._prepare_distill_targets(targets, device)

        def _scheduled_weight(name: str, base: float) -> float:
            sched = self.distill_schedule.get(name)
            if not sched:
                return base
            se = sched.get('start_epoch')
            re = sched.get('ramp_epochs')
            sw = float(sched.get('start_weight', 0.0))
            ...
            if epoch < se:
                ...
                progress = (epoch - se + 1) / float(re)
                progress = max(0.0, min(1.0, progress))
                factor = sw + (1.0 - sw) * progress
            else:
                factor = 1.0
            return base * factor
        ...
        if use_amp:
            with torch.autocast(device_type=device_type, dtype=torch.float16):
                for pair in outputs['distill_pairs']:
                    name = pair.get('name')
                    if name not in self.distill_losses:
                        continue
                    loss_module = self.distill_losses[name]
                    base_w = self.distill_meta[name]['weight']
                    weight = _scheduled_weight(name, base_w)
                    if weight <= 0.0:
                        continue
                    loss_value = loss_module(pair['student_feat'], pair['teacher_feat'], gt_boxes, img_metas)
                    distill_losses[f'loss_fgd_{name}'] = loss_value * weight
        else:
            for pair in outputs['distill_pairs']:
                name = pair.get('name')
                if name not in self.distill_losses:
    ...
    def _prepare_distill_targets(self, targets, device):
        gt_boxes = []
        img_metas = []
        for target in targets:
            ...
            if boxes is None or boxes.numel() == 0:
                ...
                if size is None:
                    raise ValueError('targets must contain `orig_size` or `size`.')
                if torch.is_tensor(size):
                    size_tensor = size.to(device=boxes.device, dtype=boxes.dtype)
                else:
                    size_tensor = torch.tensor(size, device=boxes.device, dtype=boxes.dtype)
                scale = torch.tensor([size_tensor[1], size_tensor[0], size_tensor[1], size_tensor[0]],
                                     device=boxes.device, dtype=boxes.dtype)
                gt_boxes.append(boxes_xyxy * scale)

            img_shape = target.get('img_shape', target.get('orig_size', target.get('size')))
            if img_shape is None:
                raise ValueError('targets must contain `orig_size` or `img_shape`.')
        ...
        gt_boxes = [g.to(device=device) for g in gt_boxes]
        return gt_boxes, img_metas
