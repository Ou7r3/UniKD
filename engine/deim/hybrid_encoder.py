...
class RepNCSPELAN4(nn.Module):
    ...
    def __init__(self, c1, c2, c3, c4, n=3,
        ...
        CSPLayerImpl = CSPLayer2 if csp_type == 'csp2' else CSPLayer
        self.cv2 = nn.Sequential(
            CSPLayerImpl(c3//2, c4, n, 1, bias=bias, act=act, bottletype=VGGBlock),
            ConvNormLayer_fuse(c4, c4, 3, 1, bias=bias, act=act)
        )
        self.cv3 = nn.Sequential(
            CSPLayerImpl(c4, c4, n, 1, bias=bias, act=act, bottletype=VGGBlock),
            ConvNormLayer_fuse(c4, c4, 3, 1, bias=bias, act=act)
        )
...
class HybridEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        for in_channel in in_channels:
            if in_channel != hidden_dim or self.force_proj:
        ...
        if self.num_encoder_layers > 0:
            for i, enc_ind in enumerate(self.use_encoder_idx):
                ...
                if self.training or self.eval_spatial_size is None:
                    # Build position embedding using actual feature resolution (H, W)
                    pos_embed = self.build_2d_sincos_position_embedding(
                        w, h, self.hidden_dim, self.pe_temperature).to(src_flatten.device)
                else:
                    # Use cached position embedding, but fall back to dynamic one if resolution mismatch
                    pos_embed = getattr(self, f'pos_embed{enc_ind}', None)
                    if pos_embed is not None:
                        pos_embed = pos_embed.to(src_flatten.device)
                        if pos_embed.shape[1] != src_flatten.shape[1]:
                            # Rebuild with current resolution to avoid size mismatch
                            pos_embed = self.build_2d_sincos_position_embedding(
                                w, h, self.hidden_dim, self.pe_temperature).to(src_flatten.device)
                    else:
                        pos_embed = self.build_2d_sincos_position_embedding(
